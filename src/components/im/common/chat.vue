  ...
  <div class="chat-wrapper">
    ...
    <div class="msg-container">
        <template v-if="!$common.isEmpty(currentChatFriendId) && imMessages && imMessages[currentChatFriendId]">
          <template v-for="(item, index) in imMessages[currentChatFriendId]" :key="item.id || `msg-${index}`">
            ...
            <!-- 普通消息 -->
            <div v-else :class="['msg-one', item.fromId === $store.state.currentUser.id ? 'message-right' : 'message-left']">
              <!-- 头像 -->
              <div>
                <el-avatar fit="cover"
                          :size="40"
    ...
    <div class="chat-input-area">
      ...
      <div style="padding: 10px 15px;display: flex;align-items: center;">
        ...
        <!-- 舔狗日记（已隐藏） -->
        <!-- <el-popconfirm :show-icon="false" positive-text="确认" @positive-click="sendPoetry(3)" negative-text="取消">
          <template #trigger>
            <div class="myEmoji">
              <svg viewBox="0 0 1024 1024" width="25" height="25">
                <path
        ...
        <!-- 鸡汤来咯（已隐藏） -->
        <!-- <el-popconfirm :show-icon="false" positive-text="确认" @positive-click="sendPoetry(4)" negative-text="取消">
          <template #trigger>
            <div class="myEmoji">
              <svg viewBox="0 0 1024 1024" width="25" height="25">
                <path
        ...
        <!-- 社会语录（已隐藏） -->
        <!-- <el-popconfirm :show-icon="false" positive-text="确认" @positive-click="sendPoetry(5)" negative-text="取消">
          <template #trigger>
            <div class="myEmoji">
              <svg viewBox="0 0 1024 1024" width="25" height="25">
                <path
        ...
        <!-- 诗词炮弹（已隐藏） -->
        <!-- <el-popconfirm :show-icon="false" positive-text="确认" @positive-click="sendPoetry(1)" negative-text="取消">
          <template #trigger>
            <div class="myEmoji">
              <svg viewBox="0 0 1024 1024" width="25" height="25">
                <path
      ...
      <div class="message-send">
        ...
        <el-button @click="doSend()" type="info">
          发送
        </el-button>
      </div>
    </div>
