...
<script setup>
  ...
  // 新增：响应式存储当前选中的导航Key（从本地存储读取初始值）
  const activeKey = ref(localStorage.getItem('activeNavKey') || (menuItems.length > 0 ? menuItems[0].key : ''))
  ...
  // 菜单点击（新增：存储选中Key到本地 + 激活选中状态）
  const handleMenuClick = key => {
    activeKey.value = key // 更新选中状态
    localStorage.setItem('activeNavKey', key) // 持久化到本地存储
    scrollToSection(key)
    if (isMobile.value) {
      sidebarOpen.value = false
    }
  ...
  function getImageSrc(cover) {
    ...
    return new URL(`${baseUrl}${cover}`, import.meta.url).href
  }
  ...
  onMounted(() => {
    ...
    if (activeKey.value) {
      // 加微延迟确保DOM渲染完成
      setTimeout(() => scrollToSection(activeKey.value), 100)
    }
  })
